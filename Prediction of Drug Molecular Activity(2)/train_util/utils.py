...
def get_2d(data_pd,y_label,my_dataset,max_atoms=MAX_ATOMS):
    out_file_path_train = f"./{my_dataset}_Intermediate/X_train_d2feature.npz"
    out_file_path_valid = f"./{my_dataset}_Intermediate/X_valid_d2feature.npz"
    out_file_path_test = f"./{my_dataset}_Intermediate/X_test_d2feature.npz"
    ...
    if my_dataset=="bace" or my_dataset=="BBBP":
        train_end = data_pd[data_pd['split'] == 'train'].index[-1]
        valid_end = data_pd[data_pd['split'] == 'valid'].index[-1]
        X_train = data_pd[:train_end + 1]
        X_valid = data_pd[train_end + 1:valid_end + 1]
        X_test = data_pd[valid_end + 1:]

    else:
        X_train, X_valid_test, _, y_valid_test = train_test_split(data_pd, y_label, test_size=0.2, stratify=y_label, random_state=27)
        X_valid, X_test, _, _ = train_test_split(X_valid_test, y_valid_test, test_size=0.5, stratify=y_valid_test, random_state=27)


    # （2）
    extract_save_info(X_train,out_file_path_train,max_atoms)
    extract_save_info(X_valid, out_file_path_valid, max_atoms)
    extract_save_info(X_test,out_file_path_test,max_atoms)

    # （3）
    node_embedding(out_file_path_train,out_file_path_valid,out_file_path_test,my_dataset)

    #（4）
    graph_embedding(out_file_path_train,out_file_path_valid,out_file_path_test,my_dataset,max_atoms)


    #The main function calls this function, so the relative path is the path relative to main.py
    #Delete intermediate files
    files_path = [f"./{my_dataset}_Intermediate/CBoW_50dim.pt", f"./{my_dataset}_Intermediate/graph_embedding_test.npz",
                  f"./{my_dataset}_Intermediate/graph_embedding_valid.npz",
                  f"./{my_dataset}_Intermediate/graph_embedding_train.npz",
                  f"./{my_dataset}_Intermediate/X_test_d2feature.npz",
                  f"./{my_dataset}_Intermediate/X_valid_d2feature.npz",
...
def node_embedding(train_path,valid_path,test_path,my_dataset):
    epochs = 20
    ...
    valid_dataset = GraphDataset(valid_path, padding_size=padding_size)
    test_dataset = GraphDataset(test_path, padding_size=padding_size)
    ...
    train(model,train_dataloader,optimizer,epochs,weight_file)

    test(train_dataloader,model)
    test(valid_dataloader, model)
...
def get_GraphDataset_data(data_path, padding_size,max_atom_num=MAX_ATOMS):
    FEATURE_NUM=42
    segmentation_list = [range(0, 10), range(10, 17), range(17, 24), range(24, 30), range(30, 36),
                         range(36, 38), range(38, 40), range(40, 42)]
...
#（4）边嵌入
def graph_embedding(train_path,valid_path,test_path,my_dataset,max_atoms):
    ...
    for embedding_dimension in embedding_dimension_list:
        ...
        for i in ['train','valid','test']:
            #data_path = 'D:\Pythonnnn\计算机辅组药物设计\\n_gram_graph-master\casp\data\X_{}.csv.gz'.format(i)
            if i=='train':
            ...
            out_file_path = f"./{my_dataset}_Intermediate/graph_embedding_{i}"
            np.savez_compressed(out_file_path, **kwargs)
    n_gram_num=6

    X_train = extract_feature_and_label_npy([ f"./{my_dataset}_Intermediate/graph_embedding_train.npz"],
                                            feature_name='embedded_graph_matrix_list',
                                            n_gram_num=n_gram_num)
    X_valid = extract_feature_and_label_npy([f"./{my_dataset}_Intermediate/graph_embedding_valid.npz"],
                                           feature_name='embedded_graph_matrix_list',
                                           n_gram_num=n_gram_num)
    X_test = extract_feature_and_label_npy([ f"./{my_dataset}_Intermediate/graph_embedding_test.npz"],
                                           feature_name='embedded_graph_matrix_list',
                                           n_gram_num=n_gram_num)
    with open(f"./{my_dataset}_Intermediate/X_train_d2.pkl", "wb") as f:
        pickle.dump(X_train, f)
    with open(f"./{my_dataset}_Intermediate/X_valid_d2.pkl", "wb") as f:
...
def get_walk_representation(dataloader,model):
    ...
    for batch_id, (node_attribute_matrix, adjacent_matrix, distance_matrix) in enumerate(dataloader):
        ...
        # # use cuda (4)
        if torch.cuda.is_available():
            tilde_node_attribute_matrix = tilde_node_attribute_matrix.cuda()
            embedded_graph_matrix = embedded_graph_matrix.cuda()
