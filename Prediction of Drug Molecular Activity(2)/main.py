...
import paddle
from config import my_dataset, ALPHA, MY_SCHEDULER, PATIENCE, LR, EPOCH,dataset_path
...
def train_begin(my_dataset, batch_size=4):
    ...
    train_dataset = InMemoryDataset(
        npz_data_path=f"./{my_dataset}_Intermediate/X_train_d3.npz")
    valid_dataset = InMemoryDataset(
        npz_data_path=f"./{my_dataset}_Intermediate/X_valid_d3.npz")
    ...
    logger.info("The validset size is {}, and a {} batch is required to complete the testing".format(valid_data_size, valid_need_batch))
    logger.info("The testset size is {}, and a {} batch is required to complete the testing".format(test_data_size, test_need_batch))
    ...
    for i in range(EPOCH):
        ...
        if valid_auc_epoch > max_auc:
            ...
            with open(f"./{my_dataset}_Intermediate/ROC/{valid_auc_epoch}.pkl", "wb") as file:
                pickle.dump(list(score), file)
        else:
            patience+=1
        ...
        #      norm=norm, loss_fn=loss_fn, loss_fn2=loss_fn2, test_data_size=test_data_size, y_true_test=y_true_test,
        #      logger=logger,test_auc=test_auc,epoch_num=i,mode="test")
...
if __name__ == '__main__':
    ...
    # splitter: random
        ...
        X_train, X_valid_test, y_train, y_valid_test = train_test_split(list_mol, label_list, test_size=0.2, stratify=label_list,
                                                            random_state=27)
    ...
    dataset = InMemoryDataset(
        npz_data_files=[f"./{my_dataset}_Intermediate/part1.npz"])
    data = dataset._load_npz_data_files([f"./{my_dataset}_Intermediate/part1.npz"])



    dataset = InMemoryDataset(
        npz_data_files=[f"D:\Pythonnnn\multi_fusion_bioactivity\ABL_Intermediate\part1.npz"])
    data = dataset._load_npz_data_files(["D:\Pythonnnn\multi_fusion_bioactivity\ABL_Intermediate\part1.npz"])

    for i in [f"./{my_dataset}_Intermediate/part1.npz"]:
        if os.path.exists(i):
            os.remove(i)
    ...
    if my_dataset=="BACE" or my_dataset=="BBBP":
        ...
        print("This dataset used splitter is random")
        X_train, X_valid_test, _, y_valid_test = train_test_split(data, df['label'], test_size=0.2, stratify=df['label'], random_state=27)
    ...
    train_begin(my_dataset=my_dataset)
