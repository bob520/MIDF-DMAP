...
import paddle
from config import my_dataset, ALPHA, USE_CUDA
...
def model_test(model,test_dataloader,geo_loader_test,compound_encoder,norm,test_data_size,y_true_test,lossfn1):
    test_epoch_true = 0
    ...
    with torch.no_grad():
        for data, geo_data in zip(test_dataloader, geo_loader_test):
            ...
            _, outputs, _ = model_forward(model, lossfn1, None, gram, mol_vec, graph_repr, targets, index,mode="test")
    ...
    print(
        f"The total number of hits on the test set is {test_epoch_true}/{test_data_size}，acc is{test_epoch_acc}"
    ...
    #save test ROC
    if not os.path.exists(f"./{my_dataset}_Intermediate/test_ROC"):
        os.makedirs(f"./{my_dataset}_Intermediate/test_ROC")
    with open(f"./{my_dataset}_Intermediate/test_ROC/{test_auc_epoch}.pkl", "wb") as file:
...
if __name__ == '__main__':
    ...
    model_test(model, test_dataloader=test_dataloader, geo_loader_test=geo_loader_test,
         compound_encoder=compound_encoder,
         norm=norm, y_true_test=y_true_test,test_data_size=len(test_set),lossfn1=loss_fn)